...
function get_persons( $id ) {
    ...
    if( is_array($id) ) {
        ...
        $querywhere = "WHERE rutpaciente IN (" . implode( ',', $id ) . ")";
    } else {
        $id = intval($id);
        $querywhere = "WHERE rutpaciente = " . $id;
    }
    
    if ( $result = $database->query( "SELECT concat(nombre,' ',apellidopaterno,' ',apellidomaterno)as nombre FROM mae_paciente " . $querywhere ) ) {
